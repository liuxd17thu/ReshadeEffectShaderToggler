...
struct __declspec(novtable) ResourceViewData final {
    constexpr ResourceViewData()
      : resource({ 0 })
      , format(reshade::api::format::unknown) {}
    constexpr ResourceViewData(reshade::api::resource r, reshade::api::format f)
      : resource(r)
      , format(f) {}
...
class __declspec(novtable) RenderingManager final {
  public:
    static const ResourceViewData GetCurrentResourceView(reshade::api::command_list* cmd_list,
  ...
  private:
    ...
    static bool ValidFormat(reshade::api::effect_runtime* runtime,
