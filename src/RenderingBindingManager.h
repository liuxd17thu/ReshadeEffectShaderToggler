...
class __declspec(novtable) RenderingBindingManager final {
  public:
    ...
    bool CreateTextureBinding(reshade::api::effect_runtime* runtime,
    ...
    uint32_t UpdateTextureBinding(reshade::api::effect_runtime* runtime,
                                  ShaderToggler::ToggleGroup* group,
                                  reshade::api::resource res,
                                  const reshade::api::resource_desc& desc,
                                  reshade::api::format viewformat);
    void InitTextureBingings(reshade::api::effect_runtime* runtime);
    void DisposeTextureBindings(reshade::api::device* device, std::unordered_map<int, ShaderToggler::ToggleGroup>& groups);
  ...
  private:
    ...
    bool _CreateTextureBinding(reshade::api::effect_runtime* runtime,
                               ...
                               uint32_t height,
                               uint16_t levels);
    void _QueueOrDequeue(reshade::api::command_list* cmd_list,
                         DeviceDataContainer& deviceData,
