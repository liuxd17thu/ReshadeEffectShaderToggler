...
namespace Rendering {
enum ResourceShimType { Resource_Shim_None = 0, Resource_Shim_SRGB, Resource_Shim_FFXIV };

static const std::vector<std::string> ResourceShimNames = { "none", "srgb", "ffxiv" };
...
class __declspec(novtable) ResourceManager final {
  public:
    ...
    void OnInitResourceView(reshade::api::device* device,
                            reshade::api::resource resource,
                            reshade::api::resource_usage usage_type,
    ...
    const std::shared_ptr<GlobalResourceView>& GetResourceView(reshade::api::device* device,
                                                               uint64_t handle,
